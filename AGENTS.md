...
- Releases: only the owner cuts them, with `release.ps1 -Version X.Y.Z` from an elevated shell (bumps csproj/App.cs/build.ps1/iss, builds + self-tests, commits, tags `vX.Y.Z`, pushes; Actions publishes the GitHub Release). `-DryRun` validates without writing.
