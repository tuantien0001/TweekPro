...
- Layout preview without a real Windows session: `mono bin/Release/net48/TweekPro-0.7.exe --preview <remnants|apps|stale|explorer|autorun|tools|junk|network|health|store|services|ai> [--show]` (`--show` opens the live window instead of writing `TweekPro-preview.png`). Under Mono set `MONO_WINFORMS_XIM_STYLE=disabled` and avoid keyboard input. `--preview all [dir] [--en]` renders every tab at 1280×820 into `dir` (Vietnamese unless `--en`); regenerate `docs/screenshots/` with it whenever a tab layout changes, because README embeds those PNGs.
- Releases: only the owner cuts them, with `release.ps1` from an elevated shell (auto next patch `0.7.n`, or `-Version X.Y.Z`; bumps csproj/App.cs/build.ps1/iss, builds + self-tests, commits, tags `vX.Y.Z`, pushes; Actions publishes the GitHub Release). `-DryRun` validates without writing.
