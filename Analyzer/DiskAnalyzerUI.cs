...
namespace TweekPro {
 public partial class MainForm {
  ...
  void BuildAnalyzerTab(){
   ...
   analyzerRoot=DefaultAnalyzerRoot();analyzerRootLabel.Text=Core.L.T("Thư mục phân tích: ")+(String.IsNullOrEmpty(analyzerRoot)?Core.L.T("(chưa chọn)"):analyzerRoot);
  ...
  void ChooseAnalyzerRoot(){
   using(var dialog=new FolderBrowserDialog{Description=Core.L.T("Chọn thư mục để phân tích dung lượng"),ShowNewFolderButton=false}){
    if(!String.IsNullOrEmpty(analyzerRoot)&&Directory.Exists(analyzerRoot))dialog.SelectedPath=analyzerRoot;
    if(dialog.ShowDialog(this)!=DialogResult.OK)return;
    analyzerRoot=dialog.SelectedPath;analyzerRootLabel.Text=Core.L.T("Thư mục phân tích: ")+analyzerRoot;
   }
  }

  void UpdateAnalyzerSummary(){
   analyzerSummary.Text=Core.L.F("{0} trong {1} tệp, {2} thư mục con",Presentation.BytesLabel(analyzerReport.TotalBytes),analyzerReport.TotalFiles.ToString("N0"),analyzerReport.TotalFolders.ToString("N0"))+(analyzerReport.Partial?Core.L.T("  •  (chưa đủ — đã chạm giới hạn)"):"")+Core.L.T("  •  Nhấp đúp một dòng để mở vị trí.");
  }
  ...
  async Task AnalyzeDisk(){
   if(String.IsNullOrEmpty(analyzerRoot))throw new IOException(Core.L.T("Chọn một thư mục để phân tích trước."));
   analyzerCancellation=new CancellationTokenSource();var token=analyzerCancellation.Token;
   analyzerStage.Visible=true;analyzerStage.Text=Core.L.T("Đang phân tích…");Theme.SetOverlay(analyzerOverlay,null,NoteKind.Info);
   Log(Core.L.F("Phân tích ổ đĩa: đang quét (chỉ đọc) {0}.",analyzerRoot));
   string root=analyzerRoot;
   try{
    analyzerReport=await Task.Run(()=>DiskAnalyzer.Analyze(root,200,token,ReportAnalyze),token);
    RenderAnalyzer();
    analyzerStage.Text=Core.L.F("Xong: {0} trong {1} tệp",Presentation.BytesLabel(analyzerReport.TotalBytes),analyzerReport.TotalFiles.ToString("N0"))+(analyzerReport.Partial?Core.L.T("  •  (chưa đủ)"):"");
    Log(Core.L.F("Phân tích ổ đĩa: {0}, {1} tệp, {2} nhóm thư mục.",Presentation.BytesLabel(analyzerReport.TotalBytes),analyzerReport.TotalFiles.ToString("N0"),analyzerReport.Folders.Count));
    if(analyzerReport.TotalFiles==0)Theme.SetOverlay(analyzerOverlay,"Thư mục này không có tệp nào đọc được.",NoteKind.Info);
   }catch(OperationCanceledException){analyzerStage.Text=Core.L.T("Đã dừng phân tích.");}
   finally{analyzerCancellation.Dispose();analyzerCancellation=null;}
  }

  void ReportAnalyze(AnalyzeProgress p){
   if(IsDisposed||!IsHandleCreated)return;
   try{BeginInvoke((Action)(()=>{if(!IsDisposed)analyzerStage.Text=Core.L.T(p.Stage)+(String.IsNullOrEmpty(p.Current)?"":"   "+Presentation.ShortPath(p.Current,70))+(p.Files>0?"   •   "+Core.L.F("{0} tệp, {1}",p.Files.ToString("N0"),Presentation.BytesLabel(p.Bytes)):"");}));}catch(InvalidOperationException){}
  }
  ...
  void RenderAnalyzer(){
   ...
   foreach(var folder in analyzerReport.Folders.Take(200)){
    var row=new ListViewItem(new[]{Presentation.ShortPath(folder.Path,80),Presentation.BytesLabel(folder.Bytes),Core.L.F("{0} tệp",folder.Files.ToString("N0"))}){Tag=folder,ToolTipText=folder.Path};
    Theme.AssignGroup(analyzerList,row,"1-folder",Core.L.T("Thư mục con nặng nhất"));Theme.StripeRow(row,analyzerList.Items.Count);analyzerList.Items.Add(row);
   }
   foreach(var ext in analyzerReport.Extensions.Take(80)){
    var row=new ListViewItem(new[]{ext.Extension,Presentation.BytesLabel(ext.Bytes),Core.L.F("{0} tệp",ext.Files.ToString("N0"))}){Tag=ext,ToolTipText=Core.L.T("Tổng dung lượng của các tệp ")+ext.Extension};
    Theme.AssignGroup(analyzerList,row,"2-ext",Core.L.T("Dung lượng theo phần mở rộng"));Theme.StripeRow(row,analyzerList.Items.Count);analyzerList.Items.Add(row);
   }
   foreach(var file in analyzerReport.LargestFiles.Take(200)){
    ...
    Theme.AssignGroup(analyzerList,row,"3-file",Core.L.T("Tệp lớn nhất"));Theme.StripeRow(row,analyzerList.Items.Count);analyzerList.Items.Add(row);
   }
  ...
  void OpenAnalyzerLocation(bool quiet=false){
   if(analyzerList.SelectedItems.Count==0)throw new IOException(Core.L.T("Chọn một dòng thư mục hoặc tệp."));
   ...
   else if(!quiet)throw new IOException(Core.L.T("Dòng theo phần mở rộng không có vị trí cụ thể để mở."));
  }

  void ExportAnalyzer(){
   if(analyzerReport.TotalFiles==0)throw new IOException(Core.L.T("Chưa có kết quả để xuất. Hãy Phân tích trước."));
   ...
   File.WriteAllLines(path,lines,new UTF8Encoding(true));Log(Core.L.T("Đã xuất phân tích ổ đĩa: ")+path);
  }
 }
}
