...
| **Dấu vết** | Dấu vết riêng tư của Windows (Gần đây, Jump Lists, lịch sử Run, TypedPaths, RecentDocs, hộp thoại Mở/Lưu, tìm kiếm Explorer, UserAssist, Activity History) và trình duyệt (lịch sử, cookie, biểu mẫu của Chrome/Edge/Brave/Firefox); xem trước theo quy tắc rồi dọn | Tệp vào Kho (`Kind=Tracks`), giá trị Registry lưu nguyên cây và khôi phục kiểu gộp (không ghi đè mục mới hơn); `TracksSafety` chỉ cho các khóa HKCU và thư mục trong danh sách; quy tắc trình duyệt khóa khi trình duyệt còn chạy |
...
| **Dung lượng hệ thống** | Đo Windows.old, WinSxS, bộ đệm Delivery Optimization, driver cũ trùng lặp trong DriverStore, hiberfil.sys, pagefile.sys, dung lượng điểm khôi phục, Thùng rác; mỗi mục giải phóng bằng đúng công cụ Windows (cleanmgr sageset, `DISM /StartComponentCleanup`, `pnputil /delete-driver` không `/force`, `powercfg -h off`, cmdlet) | Không tự xóa tệp trong các vùng này và **không qua Kho** (quá lớn / không hoàn tác được); mọi lệnh hỏi xác nhận và cần quyền quản trị; kết quả công cụ ghi vào nhật ký |
...
| **Tiện ích trình duyệt** | Liệt kê tiện ích của Chrome, Edge, Brave, Vivaldi, Opera, Firefox trong mọi hồ sơ: trạng thái bật/tắt, nguồn cài (cửa hàng / ép cài bằng chính sách / giải nén / cài ngoài), quyền rộng (`<all_urls>`, cookies, webRequest, history…), ngày cài, phiên bản; mở thư mục tiện ích, mở trang quản lý của trình duyệt, CSV | **Chỉ đọc** — gỡ bằng trang quản lý của chính trình duyệt để hồ sơ không hỏng; đọc `Preferences`/`Secure Preferences`/`extensions.json` bằng bản sao khi trình duyệt đang khóa tệp |
