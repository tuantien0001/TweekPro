...
| **Công cụ** | Lối mở công cụ Windows với icon hệ thống thật từ .exe/.cpl/.msc và cột dòng lệnh; Kiểm tra cập nhật; **Shortcut hỏng…**: quét Desktop/Start Menu/Startup, chọn tất cả và dọn vào Kho | SFC/chkdsk hỏi xác nhận; shortcut được kiểm tra lại trước khi dọn, khôi phục không ghi đè |
...
- **Theo dõi cài đặt**: trên tab Ứng dụng, bấm một lần để ghi danh sách hiện có. Cửa sổ thu nhỏ và có thông báo đang theo dõi. Cài phần mềm, mở lại, bấm **Kết thúc theo dõi** để thấy ứng dụng mới và có thể quét phần còn sót.
...
- **Hunter**: thu cửa sổ về khay và hiện kính ngắm đỏ. Kéo kính lên biểu tượng desktop hoặc cửa sổ để xem ứng dụng, rồi gỡ, kết thúc tiến trình hoặc mở thư mục. Chuột phải trên kính để đổi kích thước, độ trong suốt, luôn nổi, tự chạy cùng Windows, hoặc thoát.
- **Ứng dụng**: lần đầu mở tab, dòng đầu được chọn và ô phía dưới hiện thông tin ứng dụng đó.
...
- **Dung lượng** (`Sizing/`): app không khai báo `EstimatedSize` được đo từ thư mục cài, hoặc từ thư mục chứa trình gỡ / DisplayIcon khi không khai báo thư mục; game Steam đọc `libraryfolders.vdf` + `appmanifest_*.acf` nên game đã chuyển sang ổ khác (D:\SteamLibrary) hiện đúng kích cỡ và thư mục thật; tab **Ứng dụng Windows** có cột **Dung lượng** đo từ thư mục gói, đi theo liên kết `WindowsApps → D:\XboxGames` của game Xbox. Không đo Program Files, ProgramData, Windows, thư mục người dùng hay gốc ổ đĩa; mỗi thư mục tối đa 6 giây, dấu «+» nghĩa là đo chưa hết.
