...
namespace AppCare {
 public partial class MainForm {
  ...
  Label autorunNote=new Label();Button cancelScan;
  ...
  void BuildAdvancedTabs(){
   ...
   var bar=Bar();bar.WrapContents=true;bar.Height=100;
   Add(bar,"Tải danh sách",async()=>await LoadAutoruns());
   Add(bar,"Tắt (sao lưu)",async()=>await DisableAutorun());
   ...
   autorunNote.Text="Run, RunOnce và shortcut Startup của tài khoản hiện tại/toàn máy. Có đăng ký không có nghĩa Windows đang cho chạy. Bật lại áp dụng cho mục đã tắt bằng AppCare.";
   autorunNote.Dock=DockStyle.Top;autorunNote.Height=56;autorunNote.Padding=new Padding(12);autorunNote.BackColor=Color.FromArgb(230,242,255);
   ...
   var note=new Label{Text="Chỉ mở công cụ khi bạn chọn. Công cụ thiếu trên phiên bản Windows hiện tại sẽ được đánh dấu. SFC có thể sửa file hệ thống và yêu cầu quyền quản trị.",Dock=DockStyle.Top,Height=52,Padding=new Padding(12),BackColor=Color.FromArgb(230,242,255)};
   ...
   cancelScan=new Button{Text="Dừng quét sâu",Dock=DockStyle.Right,Width=135,Visible=false,BackColor=Color.FromArgb(255,237,218)};
   cancelScan.Click+=(s,e)=>{if(scanCancellation!=null)scanCancellation.Cancel();};status.Controls.Add(cancelScan);
  }
  void FillTools(){toolList.Items.Clear();foreach(var t in WindowsTools.Catalog())toolList.Items.Add(new ListViewItem(new[]{t.Name,t.Available?"Có sẵn":"Không có",t.Description}){Tag=t,ForeColor=t.Available?Color.FromArgb(38,53,74):Color.Gray});}
  ...
  void ShowAutoruns(List<AutorunEntry> entries){autorunList.BeginUpdate();autorunList.Items.Clear();startupIcons.Images.Clear();var imageHandle=startupIcons.Handle;int index=0;foreach(var a in entries){string target=Advanced.CommandExe(a.Command);if(target==""&&Advanced.LocalPath(a.Command))target=a.Command;using(var bitmap=Presentation.AppIcon(new AppEntry{DisplayIcon=target}))startupIcons.Images.Add(bitmap);autorunList.Items.Add(new ListViewItem(new[]{a.Name,a.State,a.Command,a.Source}){Tag=a,ImageIndex=index,BackColor=index++%2==0?Color.White:Color.FromArgb(247,249,252)});}autorunList.EndUpdate();}
